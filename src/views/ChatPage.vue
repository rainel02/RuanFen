      ...
      <div class="chat-sidebar">
        ...
        <div class="conversation-list" v-loading="loadingConversations">
          <div
            ...
            :class="{ active: currentConversationId === conv.withUser.userId }"
            @click="selectConversation(conv)"
          >
            <el-badge :value="conv.unreadCount" :hidden="!conv.unreadCount" class="avatar-badge">
              <el-avatar :src="conv.withUser.avatar || getRetroAvatar(conv.withUser.username || conv.withUser.name)" shape="square" />
            </el-badge>
            <div class="conv-info">
              <div class="conv-top">
                <span class="conv-name">{{ conv.withUser.username || conv.withUser.name }}</span>
                <span class="conv-time">{{ formatTime(conv.lastMessageTime) }}</span>
              </div>
              <div class="conv-last-msg">{{ conv.latestMessage }}</div>
            </div>
          </div>
        </div>
      </div>
...
const filteredConversations = computed(() => {
  ...
  return conversations.value.filter(c => (c.withUser.username || c.withUser.name || '').toLowerCase().includes(searchQuery.value.toLowerCase()))
})
...
const selectConversation = async (conv: any) => {
  currentConversationId.value = conv.withUser.userId
  currentConversationName.value = conv.withUser.username || conv.withUser.name
  currentConversationAvatar.value = conv.withUser.avatar
  await fetchMessages()
}
...
const fetchMessages = async () => {
  ...
  try {
    ...
    const msgs = (res as any).messages || (res as any).data || res
    ...
    messages.value = msgs.map((m: any) => {
      ...
      console.log(`Msg ${m.content} - Sender: ${m.senderId} - IsOwn: ${isOwn}`)
      return {
        ...m,
        isOwn
      }
...
.chat-container {
  ...
  overflow: hidden;
  /* Remove fixed height calc, let flex handle it */
  height: auto; 
  margin-bottom: 20px;
}
...
.chat-main {
  ...
  .messages-area {
    ...
    padding: 20px 30px;
    overflow-y: auto;
    min-height: 0; /* Ensure scrolling works */
